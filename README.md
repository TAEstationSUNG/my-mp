...
- [x] 이슈 태그 키워드 정교화 + **'일자리·고용'** 라벨(취업/청년고용 키워드), 연금 '노후' 오탐 제거
- [x] 세비 카드 확장 — 월급(크게)/연간세비 + **보좌진 실제 인원(의원별)** + 차량·운영비
- [x] **청년·민생 이슈 활동 카드** (프로필 상단, 청년주거/일자리/연금 발의 요약 → "나(청년)를 위해 뭘 했나")
- [x] 프로필 사진 대신 정당색 아바타 · 데이터 새 경로(`projects\my-mp`, OneDrive 밖)

### 🗺️ 우리 동네 연결 (2026-08-01 · 지역 기능 신규)
"국회의원 ↔ 우리 동네" 양방향 연결. 지역 현안은 **직접 추적/판정하지 않고 원문(뉴스)으로 연결**(원칙 유지).
- [x] **우리 지역구 최근 소식** — Google 뉴스 RSS(키 불필요·최신순) + 이슈필터(교통/개발/문화/복지/환경/안전). "의원 활동 아님, 지역 소식" 명시 + 원문 링크
- [x] **홈 두 입구** — 🔍 내 의원 찾기 / 📰 우리 동네 소식
- [x] **동네 페이지 `/town`** — 동네 뉴스 + "이 지역 국회의원" 카드 → **알아보기 →** 프로필 연결
- [x] 뉴스 로직 공용화 `lib/news.js`
- [ ] (백로그) 의원 지역 **발언 링크** — 국회도서관 발언 빅데이터(19~22대, 의원별 발언). ⚠️공개 API 없음 → 검색 링크 연결 방식만 가능
- [ ] (백로그) 서울 **정비사업(재개발/재건축) 구별 현황** — 서울 열린데이터광장 API(OA-2253). ⚠️서울 전용·지자체 소관(참고정보로만)
- 참고: "경전철 진행상황·왜 밀리나"류 개별 사업 진행률은 표준 데이터 없음 → 뉴스로 근사(직접 추적 안 함)
